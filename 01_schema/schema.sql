...
-- Dialect: SQLite 3 (dễ chạy thử, tương thích cao với MySQL/PostgreSQL)
--
-- GHI CHÚ CẢI TIẾN so với bản thiết kế gốc (để CSDL chạy được thực tế):
--   1. Thêm bảng Bank (được tham chiếu ở Vehicle_Owner nhưng chưa được định nghĩa
--      trong bản gốc).
--   2. Các cột trạng thái (Status) đổi từ TINYINT/mã số sang VARCHAR mô tả rõ
--      nghĩa (VD: 'Pending', 'Confirmed', 'Completed'...) để dễ truy vấn và
--      trình bày kết quả — tránh phải tra bảng mapping số -> nghĩa.
--   3. Loại bỏ cột Method_ID dư thừa (trùng lặp) trong Payment_Transaction vì
--      phương thức thanh toán đã được xác định ở bảng Payment.
--   4. Sắp xếp lại thứ tự CREATE TABLE theo đúng thứ tự phụ thuộc khóa ngoại
--      (bản gốc có một số bảng tham chiếu bảng chưa tồn tại).
-- ============================================================================
